...
namespace Edwon.VR
{
    public class VRGestureRig : MonoBehaviour
    {
        ...
        void Reset()
        {
            ...
            if (gameObject.GetComponent("SteamVR_ControllerManager") || gameObject.GetComponent("SteamVR_PlayArea"))
            {
        ...
        public void BeginDetect(string neuralNet = "")
        {
            uiState = VRGestureUIState.ReadyToDetect;
            currentRecognizer = new GestureRecognizer(GestureSettings.neuralNets.Contains(neuralNet) ? neuralNet : GestureSettings.currentNeuralNet);
        }

        public void SwitchNeuralNet(string neuralNet)
        {
            if (!GestureSettings.neuralNets.Contains(neuralNet)) Debug.LogError(neuralNet + " not found.");
